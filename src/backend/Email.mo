...
module {
  ...
  public func normalize(email : Text) : Result.Result<Text, Text> {
  ...
  public func isGmail(email : Text) : Bool {
    let lower = Text.toLowercase(email);
    let trimmed = Text.trim(lower, #predicate(Char.isWhitespace));
    var normalized : Result.Result<Text, Text> = #err("not a google address");
    if (Text.endsWith(trimmed, #text "@gmail.com")) normalized := normalizeGmail(email, "@gmail.com");
    if (Text.endsWith(trimmed, #text "@googlemail.com")) normalized := normalizeGmail(email, "@googlemail.com");
